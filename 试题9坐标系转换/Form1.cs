namespace 试题9坐标系转换
{
    public partial class Form1 : Form
    {
        ...
        private void open_tool_Click(object sender, EventArgs e)
        {
            ...
            {
                FileHelper.OpenFile();
                dataGridView1.RowCount = Datacenter.nodes.Count+1;
                dataGridView1[0, 0].Value = Datacenter.nodes.Count;
                
                int i = 1;
                foreach (var n in Datacenter.nodes)
                {
                    dataGridView1[0, i].Value = n.Name;
                    dataGridView1[1, i].Value = n.x;
                    dataGridView1[2, i].Value = n.y;
                    dataGridView1[3, i].Value = n.z;
                    i++;
                }
            }
            catch (Exception)
